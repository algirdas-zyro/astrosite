  ...
  <li
    ...
    :class="{
      'block-header-item--has-desktop-dropdown':
        !areDropdownsHidden && isShownOnDesktop,
    ...
    <BlockHeaderItemContent
      ...
      @toggle="
        (isShownOnMobile = !isShownOnMobile),
          (isShownOnDesktop = !isShownOnDesktop)
      "
  ...
  computed: {
    computedStyles() {
      return {
        ...
        "--m-dropdown-background-color": this.isDropdownTransparentMobile
          ? "transparent"
...
@include zyro-media($media-grid) {
  .block-header-item {
    ...
    &__dropdown-area {
      position: unset;
      padding: 8px 0;
      margin: 8px calc(-1 * var(--padding-left)) 0
        calc(-1 * var(--padding-right));
